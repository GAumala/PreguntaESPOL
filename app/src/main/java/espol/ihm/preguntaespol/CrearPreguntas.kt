...
class CrearPreguntas {
    ...
    companion object {
        ...
        fun completarPreguntas(): ArrayList<Pregunta> {
            ...
            val respuesta3a = Respuesta("A continuación la demostración de la convolución:", 3, 1470668400) //Aqui se necesita una imagen
            pregunta3.respuestas.add(respuesta3a)

            val respuesta5a = Respuesta("Interfaz podría hacer referencia al conjunto de métodos que tiene un objeto para poder trabajar con este. Ese conjunto de métodos constituyen la interfaz del objeto (en programación orientada a objetos).", 5, 1470150000)
            val respuesta5b = Respuesta("Dispositivo capaz de transformar las señales generadas por un aparato en señales comprensibles por otro.", -3, 1470081600)
            pregunta5.respuestas.addAll(listOf(respuesta5a, respuesta5b))

            val respuesta6a = Respuesta("CF = CI(1+i)^n donde CF es el capital final, CI es el capital inicial, i es la tasas de interés y n es el plazo o número de periodos.", 5, 1469304000)
            pregunta6.respuestas.add(respuesta6a)

            val respuesta7a = Respuesta("""
            ...
            val respuesta8a = Respuesta("Aquí esta el método para encontrar la máscara:", 4, 1468699200) //Aqui tambien hace falta una imagen
            pregunta8.respuestas.add(respuesta8a)

            val respuesta9a = Respuesta("La desviación estándar o desviación típica es la raíz cuadrada de la varianza. Es decir, la raíz cuadrada de la media de los cuadrados de las puntuaciones de desviación. La desviación estándar se representa por σ.", 5, 1468785600)
